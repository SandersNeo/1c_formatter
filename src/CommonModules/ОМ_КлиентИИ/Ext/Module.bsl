...
// 
// Параметры:
//  Текст        - Строка - Текущее сообщение пользователя для отправки модели
...
//  ИсторияЧата  - Массив - История чата в формате [{role: "user|assistant", content: "текст сообщения"}, ...].
//                           Этот параметр модифицируется внутри функции:
//                           - Текущее сообщение пользователя (параметр Текст) добавляется к истории
//                           - Ответ модели также добавляется к истории
//                           Если параметр не передан (Неопределено), функция работает в режиме
//                           одиночного запроса без поддержки контекста.
// 
// Возвращаемое значение:
//  Строка - Текст ответа от модели (последнее сообщение ассистента)
//
// Примечание:
//  При использовании параметра ИсторияЧата учитывайте, что он будет изменен внутри функции.
//  Это позволяет поддерживать контекст беседы между несколькими вызовами функции.
&НаКлиенте
Функция ОтправитьЗапросOpenAI(Текст, КлючAPI, БазовыйАдрес = Неопределено, Модель = Неопределено, ИсторияЧата = Неопределено) Экспорт
	...
	// Проверяем, передана ли история чата
	Если ТипЗнч(ИсторияЧата) = Тип("Массив") Тогда
		// Добавляем текущее сообщение к истории
		Если ЗначениеЗаполнено(Текст) Тогда
			ИсторияЧата.Добавить(Новый Структура("role,content", "user", Текст));
		КонецЕсли;
		
		// Используем обновленную историю сообщений
		СтруктураЗапроса.Вставить("messages", ИсторияЧата);
	Иначе
		// Режим обратной совместимости - создаем новый массив с одним сообщением
	...
	Если ОтветJSON.choices.Количество() > 0 Тогда
		ОтветМодели = ОтветJSON.choices[0].message.content;
		
		// Если передана история чата, добавляем в неё ответ модели
		Если ТипЗнч(ИсторияЧата) = Тип("Массив") Тогда
			ИсторияЧата.Добавить(Новый Структура("role,content", "assistant", ОтветМодели));
		КонецЕсли;
	КонецЕсли;
