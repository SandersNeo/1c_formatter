...
Асинх Процедура Оформить(Команда)
	...
	Для Каждого СтрокаФайла Из ВыбранныеФайлы Цикл
		...
		Если СтрокаФайла.Форматирование Тогда
			Попытка
				ВыполнитьДействие(АдресДерева, "ОформлениеТелаМетодов", "ФорматированиеКода");
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтрШаблон(
		...
		// TODO: Документирование программного интерфейса
		...
		// TODO: Сортировка служебных методов
...
&НаСервереБезКонтекста
Процедура ВыполнитьДействие(АдресДерева, ИмяКомпонента, ИдКоманды)
