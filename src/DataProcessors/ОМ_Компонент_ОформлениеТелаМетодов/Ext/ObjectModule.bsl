...
Процедура ОформитьБлокЛогическойСтроки(БлокСтроки)
	...
	// После метки точку с запятой не ставим, а метка у нас - отдельная логическая строка
	ЭтоМетка = ТаблицаТокенов[БлокСтроки.НачИндекс].Токен = "Метка";
	Если Не ЭтоМетка Тогда
		ВывестиТекст(";");
...
Функция МассивЭлементовТекстаЛогическойСтроки(БлокСтроки)
	...
	Для Индекс = БлокСтроки.НачИндекс По БлокСтроки.КонИндекс Цикл
		...
		ИначеЕсли ТокенСтр = "Запятая" 
			...
			ИЛИ ТокенСтр = "Ждать" 
			ИЛИ ТокенСтр = "Возврат"
			ИЛИ ТокенСтр = "ВызватьИсключение"
		Тогда
			ДобавитьВМассивТекста(МассивТекста, ТекстТекущегоТокена);
			ДобавитьВМассивТекста(МассивТекста, " ");
		ИначеЕсли ТокенСтр = "Комментарий" Тогда
			ДобавитьВМассивТекста(МассивТекста, " ");
			ДобавитьВМассивТекста(МассивТекста, ТекстТекущегоТокена);
		Иначе
			ДобавитьВМассивТекста(МассивТекста, ТекстТекущегоТокена);
