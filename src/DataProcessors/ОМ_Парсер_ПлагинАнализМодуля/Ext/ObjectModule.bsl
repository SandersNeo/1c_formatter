...
// Контекст анализа
Перем СтруктураОбщиеМодули;
Перем СтруктураМодулиМенеджера;
Перем СтруктураДоступныеПоляМодуля;
Перем СтруктураМенеджерыТиповМД;
Перем Конфигурация;
Перем АнализируемыйМодуль;
Перем ЕстьКонфигурацияКонтекст;
Перем ЕстьМодульКонтекст;
...
Процедура Открыть(Парсер, Параметры) Экспорт
	...
	Если ТипЗнч(Параметры) = Тип("Структура") Тогда
		Параметры.Свойство("Конфигурация", Конфигурация);
		Параметры.Свойство("Модуль", АнализируемыйМодуль);
	КонецЕсли;
	ИнициализироватьСтруктурыМодулейКонфигурации();
	ИнициализироватьСтруктурыРеквизитовОбъекта();
		...
		Если ЭлементХвоста.Тип = Типы.ВыражениеПоле Тогда // может быть еще индекс
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ИмяГолова) // голова был НЕ вызов метода 
					...
					Если ЕстьКонфигурацияКонтекст Тогда
...
Процедура ИнициализироватьСтруктурыМодулейКонфигурации()
	Если ЗначениеЗаполнено(Конфигурация) Тогда
		СтруктураОбщиеМодули = Справочники.Модули.СтруктураОбщиеМодули(Конфигурация);
		СтруктураМодулиМенеджера = Справочники.Модули.СтруктураМодулиМенеджера(Конфигурация);
		ЕстьКонфигурацияКонтекст = Истина;
	Иначе
		СтруктураОбщиеМодули = Новый Структура;
		СтруктураМодулиМенеджера = Новый Структура;
		ЕстьКонфигурацияКонтекст = Ложь;
...
Процедура ИнициализироватьСтруктурыРеквизитовОбъекта()
	Если ЗначениеЗаполнено(АнализируемыйМодуль) Тогда
		СтруктураДоступныеПоляМодуля = Справочники.Модули.СтруктураДоступныеПоляОбъекта(АнализируемыйМодуль);
		ЕстьМодульКонтекст = Истина;
	Иначе
		СтруктураДоступныеПоляМодуля = Новый Структура;
		ЕстьМодульКонтекст = Ложь;
	КонецЕсли;
КонецПроцедуры
