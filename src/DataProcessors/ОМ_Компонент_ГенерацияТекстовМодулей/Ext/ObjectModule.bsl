...
Функция ОформитьКонтекст(СтрКонтекст)
	...
	Возврат "&" + ПеревестиКонтекст(СтрКонтекст);
КонецФункции

Функция ПеревестиАннотацию(Аннотация) Экспорт
...
Функция ОформитьАннотацию(СтрАннотация, ИмяРасширяемогоМетода)
	...
	Возврат "&" + ПеревестиАннотацию(СтрАннотация) + "(""" + ИмяРасширяемогоМетода + """)";
КонецФункции
	...
	Если КоличествоУстановленныхОпций >= Цел(ИнструкцииПрепроцессора.Количество() / 2) Тогда
		...
		Для каждого КлючИЗначение Из ИнструкцииПрепроцессора Цикл
			...
			Если КлючИЗначение.Значение = Ложь Тогда
				...
				Середина = Середина + ?(Середина = "", ПеревестиКлючевоеСлово(КлючИЗначение.Ключ), ?(АнглСинтаксис, " Or ", " Или ") + ПеревестиКлючевоеСлово(КлючИЗначение.Ключ));
...
Функция ПеревестиКонтекст(Контекст) Экспорт
...
Функция ПеревестиКлючевоеСлово(КлючевоеСлово) Экспорт
