...
Функция ТекстМодуля(СтрЭлементМодуль) Экспорт
	ВычислитьИнструкцииПрепроцессора(СтрЭлементМодуль);
...
Функция ГенерироватьТекстМетода(
	СтрЭлементМетод, 
	ВключатьКод = Истина, 
	ВключатьДокументацию = Истина, 
	ТолькоСигнатура = Ложь, 
	ВключатьПрепроцессор = Ложь, 
	_АнглСинтаксис = Ложь) Экспорт
	
	АнглСинтаксис = _АнглСинтаксис;
	Возврат МетодТекст(СтрЭлементМетод, ВключатьКод, ВключатьДокументацию, ТолькоСигнатура, ВключатьПрепроцессор);
	
КонецФункции

Функция ГенерироватьТекстИнструкцийПрепроцессора(ИнструкцииПрепроцессора, _АнглСинтаксис = Ложь) Экспорт
	АнглСинтаксис = _АнглСинтаксис;
	Возврат ГенерироватьТекстИнструкцийПрепроцессора(ИнструкцииПрепроцессора);
КонецФункции
...
Функция СигнатураМетодаHTML(СодержимоеМетод, ВключатьДокументацию = Истина, ВключатьПрепроцессор = Ложь, _АнглСинтаксис = Ложь) Экспорт
	...
	Если ВыводитьПрепроцессор Тогда
		...
		ТД.ДобавитьСтроку(ОформитьСлужебныйТекст(ПрепроцессорТекст));
	КонецЕсли;
	...
	Если ВключатьДокументацию Тогда
		...
		Если ЗначениеЗаполнено(КомментарийКМетоду) Тогда
			ТД.ДобавитьСтроку(ОформитьКомментарийHTML(КомментарийКМетоду));
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Содержимое.Контекст) Тогда
		ТД.ДобавитьСтроку(ОформитьСлужебныйТекст(ОформитьКонтекст(Содержимое.Контекст)));
	КонецЕсли;
	Если ЗначениеЗаполнено(Содержимое.Аннотация) Тогда
		ТД.ДобавитьСтроку(ОформитьСлужебныйТекст(ОформитьАннотацию(Содержимое.Аннотация, Содержимое.ИмяРасширяемогоМетода)));
	КонецЕсли;
	...
	// Асинх
	Если Содержимое.Асинх Тогда
		СигнатураМетода = СигнатураМетода + ОформитьКлючевоеСлово(ТекстАсинх()) + " ";
	КонецЕсли;
	
	// Процедура/Функция
	ТекстОбъявление = ?(Содержимое.ЭтоФункция, ТекстФункция(), ТекстПроцедура());
	СигнатураМетода = СигнатураМетода + ОформитьКлючевоеСлово(ТекстОбъявление);
	
	// Имя метода
	СигнатураМетода = СигнатураМетода + " " + ОформитьИдентификатор(Содержимое.Имя);
	
	// Параметры
	СигнатураМетода = СигнатураМетода + ОформитьСпецСимвол("(") 
		+ ОформитьПараметрыМетодаHTML(Содержимое.Параметры) 
		+ ОформитьСпецСимвол(")");
	
	// Экспорт
	Если Содержимое.Экспортная Тогда
		СигнатураМетода = СигнатураМетода + " " + ОформитьКлючевоеСлово(ТекстЭкспорт());
	КонецЕсли;
	
	// Комментарий
	Если ВключатьДокументацию И ЗначениеЗаполнено(Содержимое.КомментарийОднострочный) Тогда
		СигнатураМетода = СигнатураМетода + ОформитьКомментарийHTML(ОформитьКомментарийОднострочный(СокрЛП(Содержимое.КомментарийОднострочный)));
	КонецЕсли;
	
	ТД.ДобавитьСтроку(СигнатураМетода);
	
	Возврат ПолучитьHTML_СПараграфами(ТД);
КонецФункции
		...
		Если ВыполнятьРасстановкуДирективКомпилятора 
			...
			Если НЕ ИнструкцииПрепроцессораИдентичны(ТекущиеИнструкцииПрепроцессораМодуля, СтруктураИнструкцийПрепроцессораПоУмолчанию()) Тогда
				...
				Если ДирективыПротивоположны(ТекущиеИнструкцииПрепроцессораМодуля, ИнструкцииПрепроцессораТекущегоЭлемента) Тогда
					ТД.ДобавитьСтроку(?(АнглСинтаксис, "#Else", "#Иначе"));
					ТД.ДобавитьСтроку("");
...
Функция ТекстПрепроцессорКонецЕсли()
	Возврат ?(АнглСинтаксис, "#EndIf", "#КонецЕсли");
КонецФункции
...
#Область HTML_форматирование

Функция ОформитьПараметрыМетодаHTML(Параметры)
	СтрПараметры = "";
	
	Для Индекс = 0 По Параметры.Количество() - 1 Цикл
		СтрПараметр = Параметры[Индекс];
		
		Если Индекс > 0 Тогда
			СтрПараметры = СтрПараметры + ОформитьСпецСимвол(", ");
		КонецЕсли;
		
		Если СтрПараметр.ПоЗначению Тогда
			СтрПараметры = СтрПараметры + ОформитьКлючевоеСлово(ТекстЗнач()) + " ";
		КонецЕсли;
		
		СтрПараметры = СтрПараметры + ОформитьИдентификатор(СтрПараметр.Имя);
		
		Если ЗначениеЗаполнено(СтрПараметр.ЗначениеПоУмолчанию) Тогда
			СтрПараметры = СтрПараметры + " " + ОформитьСпецСимвол("=") + " " 
				+ ОформитьЗначениеПоУмолчанию(СтрПараметр.ЗначениеПоУмолчанию);
		КонецЕсли;
	КонецЦикла;
...
Функция ОформитьСпецСимвол(Текст)
	Возврат "<span style='color: red'>" + Текст + "</span>";
КонецФункции
...
Функция ОформитьКомментарийHTML(Текст)
	Возврат "<span style='color: green'>" + Текст + "</span>";
КонецФункции

// Преобразует текстовый документ в HTML с параграфами без отступов между ними
//
// Параметры:
//  ТД - ТекстовыйДокумент - исходный текстовый документ
//
// Возвращаемое значение:
//  Строка - полноценный HTML документ, где каждая строка обернута в параграф без отступов
//
Функция ПолучитьHTML_СПараграфами(ТД)
	// Формируем полноценный HTML документ
	Результат = "<!DOCTYPE html>
	|<html>
	|<head>
	|<meta charset='utf-8'>
	|<title>Сигнатура метода</title>
	|<style>
	|html, body {
	|	margin: 0;
	|	padding: 0;
	...
	Для НомСтр = 1 По ТД.КоличествоСтрок() Цикл
		Стр = ТД.ПолучитьСтроку(НомСтр);
		Если ЗначениеЗаполнено(Стр) Тогда
			Результат = Результат + "<p>" + Стр + "</p>";
		КонецЕсли;
	КонецЦикла;
	
	Результат = Результат + "</div></body>
	|</html>";
