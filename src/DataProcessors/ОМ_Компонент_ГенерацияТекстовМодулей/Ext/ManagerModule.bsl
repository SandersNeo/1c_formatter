...
Функция ТекстМодуля(СтрЭлементМодуль) Экспорт
	...
	Возврат ГенерироватьТекстЭлементаРек(СтрЭлементМодуль, ТекущиеИнструкцииПрепроцессораМодуля);
КонецФункции
...
Функция ГенерироватьТекстЭлементаРек(СтрЭлемент, ТекущиеИнструкцииПрепроцессораМодуля)
	ТекстЭлемента = "";
	
	Если СтрЭлемент.ТипЭлемента = "Модуль" Тогда
		
		ТекстЭлемента = ГенерироватьТекстМодуляПоМассиву(СтрЭлемент, ТекущиеИнструкцииПрепроцессораМодуля);
		
	ИначеЕсли СтрЭлемент.ТипЭлемента = "Область" Тогда
		
		ТекстЭлемента = ОбластьТекст(СтрЭлемент, ТекущиеИнструкцииПрепроцессораМодуля);
...
Функция ГенерироватьТекстМодуляПоМассиву(ГоловнойЭлемент, ТекущиеИнструкцииПрепроцессораМодуля)
...
Функция ОбластьТекст(СтрЭлемент, ТекущиеИнструкцииПрепроцессораМодуля)
	...
	Если ЗначениеЗаполнено(СтрЭлемент) Тогда
		ТД.ДобавитьСтроку(
			ГенерироватьТекстМодуляПоМассиву(СтрЭлемент, ТекущиеИнструкцииПрепроцессораМодуля)
			+ "#КонецОбласти" + ОформитьКомментарийОднострочный(КомментарийПослеКонецОбласти));
	КонецЕсли;
...
Функция ПеременнаяТекст(СтрЭлемент) 
	...
	ТД.ДобавитьСтроку(
		"Перем " + ОформитьИменаПеременных(Содержимое.ТаблицаПеременных)
		+ ";"
...
Функция МетодТекст(СтрЭлемент, ВключатьКод = Истина, ВключатьДокументацию = Истина, ТолькоСигнатура = Ложь)
	Если СтрЭлемент.ТипЭлемента = "Процедура" Тогда
		ОбъявлениеМетода = "Процедура";
		ОбъявлениеКонецМетода = "КонецПроцедуры";
	ИначеЕсли СтрЭлемент.ТипЭлемента = "Функция" Тогда
		ОбъявлениеМетода = "Функция";
		ОбъявлениеКонецМетода = "КонецФункции";
	Иначе
	...
	Если ВключатьКод Тогда
		...
		КонецЕсли;
		ТД.ДобавитьСтроку(?(Содержимое.Асинх, "Асинх ", "") + ОбъявлениеМетода + " " + Содержимое.Имя + "(" + ОформитьПараметрыМетода(Содержимое.Параметры) + ")" + ?(Содержимое.Экспортная, " Экспорт", "") + ?(ВключатьДокументацию, ОформитьКомментарийОднострочный(СокрЛП(Содержимое.КомментарийОднострочный)), ""));
...
Функция ПроцедураТекст(СтрЭлемент)
	...
	Если ЗначениеЗаполнено(Содержимое.Аннотация) Тогда
		ТД.ДобавитьСтроку(ОформитьАннотацию(Содержимое.Аннотация, Содержимое.ИмяРасширяемогоМетода));
	КонецЕсли;
	ТД.ДобавитьСтроку(?(Содержимое.Асинх, "Асинх ", "") + "Процедура " + Содержимое.Имя + "(" + ОформитьПараметрыМетода(Содержимое.Параметры) + ")" + ?(Содержимое.Экспортная, " Экспорт", "") + ОформитьКомментарийОднострочный(СокрЛП(Содержимое.КомментарийОднострочный)));
	ТД.ДобавитьСтроку(ОформитьТекстМетода(Содержимое.Тело));
	ТД.ДобавитьСтроку("КонецПроцедуры" + ОформитьКомментарийОднострочный(Содержимое.КомментарийОднострочныйКонец));
...
Функция ФункцияТекст(СтрЭлемент)
	...
	Если ЗначениеЗаполнено(Содержимое.Аннотация) Тогда
		ТД.ДобавитьСтроку(ОформитьАннотацию(Содержимое.Аннотация, Содержимое.ИмяРасширяемогоМетода));
	КонецЕсли;
	ТД.ДобавитьСтроку(?(Содержимое.Асинх, "Асинх ", "") + "Функция " + Содержимое.Имя + "(" + ОформитьПараметрыМетода(Содержимое.Параметры) + ")" + ?(Содержимое.Экспортная, " Экспорт", "") + ОформитьКомментарийОднострочный(Содержимое.КомментарийОднострочный));
	ТД.ДобавитьСтроку(ОформитьТекстМетода(Содержимое.Тело));
	ТД.ДобавитьСтроку("КонецФункции" + ОформитьКомментарийОднострочный(Содержимое.КомментарийОднострочныйКонец));
	...
	Если КоличествоУстановленныхОпций >= Цел(ИнструкцииПрепроцессора.Количество() / 2) Тогда
		...
		Для каждого КлючИЗначение Из ИнструкцииПрепроцессора Цикл
			...
			Если КлючИЗначение.Значение = Ложь Тогда
				...
				Середина = Середина + ?(Середина = "", КлючИЗначение.Ключ, " Или " + КлючИЗначение.Ключ);
