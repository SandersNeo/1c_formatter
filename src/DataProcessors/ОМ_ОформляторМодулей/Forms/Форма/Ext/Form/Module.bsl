...
Процедура ИмяПриИзменении(Элемент)
	...
	Если ТекСтрока.ТипЭлемента <> "Комментарий" 
		...
		Если ТекСтрока.ТипЭлемента = "Процедура" 
			ИЛИ ТекСтрока.ТипЭлемента = "Функция" Тогда 
			ПолеМетодаПриИзменении(Элемент);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПолеМетодаПриИзменении(Элемент)
	СформироватьСигнатуруМетодаТекстом();
КонецПроцедуры
...
Процедура КомандаДействие(Команда)
	...
	Если ПоВыбраннымСтрокам Тогда
		ЕстьВыбранныеСтроки = ОбновитьПризнакВыбораВСтрокахСтруктурыМодуля(ТекКоманда.ОбластьДействия);
		Если Не ЕстьВыбранныеСтроки Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Для выбранных строк данное действие не предусмотрено";
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьСодержимоеСтрокиСтруктуры(ЭлементДереваСтруктуры)
	...
	ИначеЕсли ЭлементДереваСтруктуры.ТипЭлемента = "Процедура"
		...
		ЗаполнитьСодержимоеМетодаИзПолейФормы(Содержимое);
...
&НаСервере
Процедура ЗаполнитьСодержимоеМетодаИзПолейФормы(Содержимое)
	Содержимое.Имя = Имя;
...
Процедура ЗагрузитьСодержимоеСтрокиСтруктуры(ЭлементДереваСтруктуры)
	...
	ИначеЕсли ЭлементДереваСтруктуры.ТипЭлемента = "Процедура"
		...
		СформироватьСигнатуруМетодаТекстом(Содержимое);
...
&НаСервере
Процедура СформироватьСигнатуруМетодаТекстом(Знач СодержимоеМетода = Неопределено)
	Если СодержимоеМетода = Неопределено Тогда
		СодержимоеМетода = Обработки.ОМ_ОформляторМодулей.МетодСтруктураОписания();
		ЗаполнитьСодержимоеМетодаИзПолейФормы(СодержимоеМетода);
	КонецЕсли;
	СигнатураТекстом = Обработки.ОМ_Компонент_ГенерацияТекстовМодулей.СигнатураМетодаHTML( 
		СодержимоеМетода,
		Ложь, 
		Истина);
КонецПроцедуры
...
Процедура ОткрытьФормуДействияЗавершение(РезультатВыполненияКоманды, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(РезультатВыполненияКоманды) = Тип("Структура") Тогда
		...
		Если РезультатВыполненияКоманды.Свойство("АдресДереваСтруктурыМодулей", АдресДереваСтруктурыМодулей) Тогда
			ЗаполнитьДеревоСтруктурыМодуляИзВременногоХранилища(АдресДереваСтруктурыМодулей);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДеревоСтруктурыМодуляИзВременногоХранилища(Адрес)

	дз = ПолучитьИзВременногоХранилища(Адрес);
...
&НаКлиенте
Функция ОбновитьПризнакВыбораВСтрокахСтруктурыМодуля(ОбластьДействия)
...
#Область ЗапредРедактированияПолей

&НаКлиентеНаСервереБезКонтекста
Функция ИменаЭлементовВажныхПолейСтруктурыМодуля(ТолькоЭлементыПолейМетода = Ложь) Экспорт

	ИменаПолей = Новый Массив;
	
	ИменаПолей.Добавить("ИмяПодпрограммы");
	ИменаПолей.Добавить("ЭтоФункция");
	ИменаПолей.Добавить("Экспорт");
	ИменаПолей.Добавить("Асинх");
	ИменаПолей.Добавить("Контекст");
	ИменаПолей.Добавить("Аннотация");
	ИменаПолей.Добавить("ИмяРасширяемойПодпрограммы");
	ИменаПолей.Добавить("ПараметрыПодпрограммыИмя");
	ИменаПолей.Добавить("ПараметрыПодпрограммыЗначениеПоУмолчанию");
	ИменаПолей.Добавить("ПараметрыПодпрограммыПоЗначению");
	ИменаПолей.Добавить("Текст1");
	ИменаПолей.Добавить("Клиент");
	ИменаПолей.Добавить("Сервер");
	ИменаПолей.Добавить("ТонкийКлиент");
	ИменаПолей.Добавить("ВебКлиент");
	ИменаПолей.Добавить("ТолстыйКлиентОбычноеПриложение");
	ИменаПолей.Добавить("ТолстыйКлиентУправляемоеПриложение");
	ИменаПолей.Добавить("ВнешнееСоединение");
	ИменаПолей.Добавить("МобильныйКлиент");
	ИменаПолей.Добавить("МобильноеПриложениеКлиент");
	ИменаПолей.Добавить("МобильноеПриложениеСервер");
	ИменаПолей.Добавить("МобильныйАвтономныйСервер");
	
	Если Не ТолькоЭлементыПолейМетода Тогда
		ИменаПолей.Добавить("СтруктураМодуляОписание");
		
		ИменаПолей.Добавить("Контекст1");
		ИменаПолей.Добавить("Переменные");
		ИменаПолей.Добавить("Клиент1");
		ИменаПолей.Добавить("Сервер1");
		ИменаПолей.Добавить("ТонкийКлиент1");
		ИменаПолей.Добавить("ВебКлиент1");
		ИменаПолей.Добавить("ТолстыйКлиентОбычноеПриложение1");
		ИменаПолей.Добавить("ТолстыйКлиентУправляемоеПриложение1");
		ИменаПолей.Добавить("ВнешнееСоединение1");
		ИменаПолей.Добавить("МобильныйКлиент1");
		ИменаПолей.Добавить("МобильноеПриложениеКлиент1");
		ИменаПолей.Добавить("МобильноеПриложениеСервер1");
		ИменаПолей.Добавить("МобильныйАвтономныйСервер1");
		
		ИменаПолей.Добавить("Текст");
		ИменаПолей.Добавить("Клиент2");
		ИменаПолей.Добавить("Сервер2");
		ИменаПолей.Добавить("ТонкийКлиент2");
		ИменаПолей.Добавить("ВебКлиент2");
		ИменаПолей.Добавить("ТолстыйКлиентОбычноеПриложение2");
		ИменаПолей.Добавить("ТолстыйКлиентУправляемоеПриложение2");
		ИменаПолей.Добавить("ВнешнееСоединение2");
		ИменаПолей.Добавить("МобильныйКлиент2");
		ИменаПолей.Добавить("МобильноеПриложениеКлиент2");
		ИменаПолей.Добавить("МобильноеПриложениеСервер2");
		ИменаПолей.Добавить("МобильныйАвтономныйСервер2");
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьРазрещениеРедактирования(Разрешить) Экспорт

	// Получаем список полей
	ИменаПолей = ИменаЭлементовВажныхПолейСтруктурыМодуля();
	
	// Устанавливаем признак ТолькоПросмотр для каждого поля
	Для Каждого ИмяПоля Из ИменаПолей Цикл
		Элементы[ИмяПоля].ТолькоПросмотр = Не Разрешить;
	КонецЦикла;
	
	Элементы.ПараметрыПодпрограммы.ИзменятьСоставСтрок = Разрешить;
	Элементы.ПараметрыПодпрограммы.ИзменятьПорядокСтрок = Разрешить;
	
	Элементы.Переменные.ИзменятьСоставСтрок = Разрешить;
	Элементы.Переменные.ИзменятьПорядокСтрок = Разрешить;
...
Процедура РазрешитьРедактирование(Команда)
	...
	Элементы.СтруктураМодуляРазрешитьРедактирование.Пометка = РазрешитьРедактирование;
	УстановитьРазрещениеРедактирования(РазрешитьРедактирование);
