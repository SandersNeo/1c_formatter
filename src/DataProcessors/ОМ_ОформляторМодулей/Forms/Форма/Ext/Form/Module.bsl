...
Процедура КомандаДействие(Команда)
	// Разбираем имя команды для получения имени обработки и идентификатора
	ИмяКоманды = Команда.Имя;
	ЧастиИмени = СтрРазделить(ИмяКоманды, "_"); // давай переделаем. лучше предположим, что у нас в КомандыДействий есть колонка ИмяОбработки, по которой и будем искать строку в этой таблице, сразу по имени команды. И ниже в процедуре где формируется эта таблица также добавить заполнения колонки ИмяКоманды AI!
	...
	Идентификатор = ЧастиИмени[2];
	
	// Ищем команду в таблице КомандыДействий
	НайденныеСтроки = КомандыДействий.НайтиСтроки(Новый Структура("ИмяОбработки,Идентификатор", ИмяОбработки, Идентификатор));
	...
	Если ТекКоманда.СпособЗапускаДействия = "ОткрытиеФормы" Тогда
		...
		ОткрытьФорму(
			"Обработка." + ИмяОбработки + ".Форма.Форма",
	...
	ИначеЕсли ТекКоманда.СпособЗапускаДействия = "ВызовМетодаМодуляМенеджера" Тогда
		...
		ВыполнитьДействие(ИмяОбработки);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ВыполнитьДействие(ИмяОбработки)
	дз = ВыгрузитьСтруктуруМодулей();
	МенеджерОбработки = Обработки[ИмяОбработки];
	МенеджерОбработки.ВыполнитьДействие(дз);
