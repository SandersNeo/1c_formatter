...
Процедура ЗаполнитьМенюДействий()
	...
	КомандыДействий.Колонки.Добавить("СпособЗапускаДействия");
	
	ОбработкиПодсистемы = Метаданные.Подсистемы.ОМ_Действия.Состав;
	
	Для Каждого МетаданныеОбработки Из ОбработкиПодсистемы Цикл
		МенеджерОбработки = РеквизитФормыВЗначение("Объект").Метаданные().Менеджер().ПолучитьМенеджер(МетаданныеОбработки.Имя);
