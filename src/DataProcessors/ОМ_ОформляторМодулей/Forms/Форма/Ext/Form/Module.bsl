...
Процедура КомандаДействие(Команда)
	// AI! Давай реализуем здесь такой функционал. Выделяем из имени команды имя обработки и идентификатор (это динамически создаваемая команда
	// в процедуре создания команд). Далее у нас есть в реквизитах формы таблица КомандыДействий с колонками ИмяОбработки, Идентификатор, СпособЗапускаДействия, ОбластьДействия, ищем 
	// в ней строку текущей коанды. Если способ запуска = СпособЗапускаДействия, то открываем основную форму обработке, передавая ей в качестве параметра
	// адрес дерева структуры модуля (см. код открытия формы генерации текста модуля в этом модуле). Если ВызовМетодаМодуляМенеджера, то 
	// вызываем сервеную процедуру ВыполнитьДействие(ИмяОбработки), которая вызывает функцию модуля менеджера обработки, имя котрой туда передается
	// передавая МенеджерОбработки.ВыполнитьДействие(ДеревоСтруктурыМодуля), передавая дерево структуры (выгруженное в дерево значений)
КонецПроцедуры
...
&НаКлиенте
Процедура ГенерироватьТекстМодуля(Команда)
	АдресДереваСтруктурыМодулей = АдресДереваСтруктурыМодулей();

	ОткрытьФорму(
		"Обработка.ОМ_Компонент_ГенерацияТекстовМодулей.Форма.ФормаВыводаТекстов",
		Новый Структура("АдресДереваСтруктурыМодулей", АдресДереваСтруктурыМодулей),
		ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура РаспределитьПоОбластям(Команда)
	РаспределитьПоОбластямНаСервере();
	
	//АдресДереваСтруктурыМодулей = АдресДереваСтруктурыМодулей();
	//
	//ОткрытьФорму(
	//	"Обработка.ОМ_Компонент_РасстановкаОбластей.Форма.Форма",
	//	Новый Структура("АдресДереваСтруктурыМодулей", АдресДереваСтруктурыМодулей),
	//	ЭтаФорма,
	//	,
	//	,
	//	,
	//	Новый ОписаниеОповещения("РаспределитьПоОбластямЗавершение", ЭтотОбъект),
	//	РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура УстановитьИнструкцииПрепроцессораДляМодуляМенеджера(Команда)
	УстановитьИнструкцииПрепроцессораДляМодуляМенеджераНаСервере();
КонецПроцедуры
...
&НаКлиенте
Процедура ГенерироватьОписаниеВозвращаемогоЗначения(Команда)
	ОткрытьФормуГенерацииОписанияМетода("ВозвращаемоеЗначение");
КонецПроцедуры

&НаКлиенте
Процедура ГенерироватьОписаниеМетода(Команда)
	ОткрытьФормуГенерацииОписанияМетода("Описание");
КонецПроцедуры

&НаКлиенте
Процедура ГенерироватьТипыИОписанияПараметров(Команда)
	ОткрытьФормуГенерацииОписанияМетода("Параметры");
КонецПроцедуры
...
&НаСервере
Процедура РаспределитьПоОбластямНаСервере()
	дз = ВыгрузитьСтруктуруМодулей();
	НовоеДеревоСтруктураМодуля = Обработки.ОМ_Компонент_РасстановкаОбластей.ДеревоСтруктурыМодулейСоСтандартнымиОбластями(дз);

	СтруктураМодуля.ПолучитьЭлементы().Очистить();
	ЗаполнитьДеревоСтруктурыМодуляРек(НовоеДеревоСтруктураМодуля, СтруктураМодуля.ПолучитьЭлементы());
КонецПроцедуры
...
&НаСервере
Процедура УстановитьИнструкцииПрепроцессораДляМодуляМенеджераНаСервере()
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ДеревоСтруктураМодуля = ВыгрузитьСтруктуруМодулей();
	
	ОбработкаОбъект.УстановитьИнструкцииПрепроцессораДляМодуляМенеджера(ДеревоСтруктураМодуля);
...
&НаКлиенте
Процедура ОткрытьФормуГенерацииОписанияМетода(ТипОписания)
	//// Получаем имя метода, который мы анализируем
	//ИмяМетода = Имя;
	//
	//// Преобразуем модуль в ДЗ
	//АдресДереваСтруктурыМодуля = АдресДереваСтруктурыМодуляПоТекущемуЭлементу();
	//Если АдресДереваСтруктурыМодуля = Неопределено Тогда
	//	Возврат;
	//КонецЕсли;

	//ПараметрыФормы = Новый Структура(
	//	"АдресДереваСтруктурыМодуля,ИмяМетодаДляОписания,ТипОписания", 
	//	АдресДереваСтруктурыМодуля,
	//	ИмяМетода,
	//	ТипОписания);
	//
	//ОткрытьФорму(
	//	"ВнешняяОбработка.РабочееМестоРазработкиМодуля.Форма.ФормаГенерацииОписанияМетода",
	//	ПараметрыФормы,
	//	ЭтаФорма,,,,
	//	Новый ОписаниеОповещения("ГенерацииОписанияМетодаЗавершение", ЭтотОбъект, ТипОписания),
	//	РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	//
КонецПроцедуры
...
Процедура ЗаполнитьМенюДействий()
	...
	СоставПодсистемыДействия = Метаданные.Подсистемы.ОМ_Действия.Состав;
