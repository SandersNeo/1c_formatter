...
// Находит область по имени в дереве структуры модуля, выполняя регистронезависимый поиск
// 
// Параметры:
//  ДеревоСтруктурыМодуля - ДеревоЗначений - Дерево структуры модуля
//  ИмяОбласти            - Строка         - Имя искомой области
// 
// Возвращаемое значение:
//  СтрокаДереваЗначений - Строка области, если найдена, иначе Неопределено
Функция НайтиОбластьПоИмени(ДеревоСтруктурыМодуля, ИмяОбласти) Экспорт
	ИскомоеИмя = ВРег(СокрЛП(ИмяОбласти));
	Возврат НайтиОбластьПоИмениРек(ДеревоСтруктурыМодуля.Строки, ИскомоеИмя);
КонецФункции
...
// Рекурсивно ищет область по имени в дереве структуры модуля
//
// Параметры:
//  СтрокиДерева - КоллекцияСтрокДереваЗначений - Строки дерева для поиска
//  ИскомоеИмя    - Строка                       - Имя области в верхнем регистре
//
// Возвращаемое значение:
//  СтрокаДереваЗначений - Найденная строка области или Неопределено
Функция НайтиОбластьПоИмениРек(СтрокиДерева, ИскомоеИмя)
	Для каждого Стр Из СтрокиДерева Цикл
		Если Стр.ТипЭлемента = "Область" И ВРег(СокрЛП(Стр.Содержимое.Имя)) = ИскомоеИмя Тогда
			Возврат Стр;
		КонецЕсли;
		
		Результат = НайтиОбластьПоИмениРек(Стр.Строки, ИскомоеИмя);
