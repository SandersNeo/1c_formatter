...
Функция КлючОбласти(ИмяОбласти, ИмяТаблицы = "")
	
	Если НЕ АнглСинтаксис Тогда
		Возврат ?(ПустаяСтрока(ИмяТаблицы), ИмяОбласти, ИмяОбласти + ИмяТаблицы);
	КонецЕсли;
	...
	ИмяОбластиПереведенное = ?(СоответствиеОбластей[ИмяОбласти] <> Неопределено,
		СоответствиеОбластей[ИмяОбласти],
		ИмяОбласти);
		
	Возврат ?(ПустаяСтрока(ИмяТаблицы), 
		ИмяОбластиПереведенное, 
		ИмяОбластиПереведенное + ИмяТаблицы);
		
КонецФункции

Функция КудаОпределяемЭлемент(ЭлементСтруктуры, ИмяОбласти, ПорядокОбласти, ПорядокПодпрограммы = 10000, ДопПорядок = "")
	
	ИмяТаблицы = ?(ИмяОбласти = "ОбработчикиСобытийЭлементовТаблицыФормы", ДопПорядок, "");
	
	Возврат Новый Структура(
		"ЭлементСтруктуры, ИмяОбласти, ПорядокОбласти, ПорядокПодпрограммы, ДопПорядок", 
		ЭлементСтруктуры, КлючОбласти(ИмяОбласти, ИмяТаблицы), ПорядокОбласти, ПорядокПодпрограммы, ДопПорядок);
