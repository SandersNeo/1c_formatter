...
Функция НайтиИлиДобавитьОбласть(СтрМодуль, ИмяОбласти)
	...
	НоваяСтрокаДерева.Описание = ИмяОбластиВРег;
	НоваяСтрокаДерева.Содержимое = Обработки.ОМ_ОформляторМодулей.ОбластьСтруктураОписания();
	НоваяСтрокаДерева.Содержимое.Имя = ИмяОбластиВРег;
...
Функция КлючОбласти(ИмяОбласти)
	...
	// давай сделаем рефакторинг кода ниже. Сначала получаем по соответствию областей значение, и если оно = Неопределено, то  в этомслучае уже обрабатываем ОбработчикиСобытийЭлементовТаблицыФормы, а если и его не находим, то возвращаем имя области как передано AI!

	// Особая обработка для областей с именами таблиц
	...
	Возврат ?(СоответствиеОбластей[ИмяОбласти] <> Неопределено,
		СоответствиеОбластей[ИмяОбласти],
		ИмяОбласти);
...
Функция КудаОпределяемЭлемент(ЭлементСтруктуры, ИмяОбласти, ПорядокОбласти, ПорядокПодпрограммы = 10000, ДопПорядок = "")
	...
	ИмяОбластиПолное = ?(ИмяОбласти = "ОбработчикиСобытийЭлементовТаблицыФормы", 
		ИмяОбласти + ДопПорядок, 
		ИмяОбласти);
