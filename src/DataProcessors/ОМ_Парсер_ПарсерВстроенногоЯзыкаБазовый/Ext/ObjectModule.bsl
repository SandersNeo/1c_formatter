...
//	ПарсерДляОформления = Обработки.ОМ_Парсер_ПарсерВстроенногоЯзыкаБазовый.Создать();
...
//
Функция СоздатьБлокЛогическойСтроки(НачИндекс, КонИндекс, ИндексКомментария = -1) Экспорт
	Возврат Новый Структура(
		"ВидБлока, НачИндекс, КонИндекс, ИндексКомментария",
		"ЛогическаяСтрока", НачИндекс, КонИндекс, ИндексКомментария
	);
КонецФункции
...
Функция РазобратьЛогическуюСтроку()
	...
	Пока ТекущийИндекс < ВсегоТокенов Цикл
		...
		Если ЭтоИнструкцияПрепроцессора 
			И (ТаблицаТокенов[ТекущийИндекс].НомерСтроки <> ТекущийНомерСтроки 
				ИЛИ ТекущийТокен = "КонецТекста")
		Тогда
			Прервать;
		КонецЕсли;
		
		// Проверка на завершающие токены для обычных строк
		Если НЕ ЭтоИнструкцияПрепроцессора 
			И ЗавершающиеТокены.Свойство(ТекущийТокен) 
		Тогда
			Прервать;
		КонецЕсли;
