...
//  КонецСчётчика - Структура - блок логической строки с конечным значением счетчика
//  Тело - Структура - блок кода, содержащий тело цикла
//
// Возвращаемое значение:
//  Структура - блок цикла Для
//
Функция СоздатьБлокДля(Инициализация, КонецСчётчика, Тело = Неопределено) Экспорт
	Если Тело = Неопределено Тогда
		Тело = СоздатьБлокКода();
	КонецЕсли;
	
	Возврат Новый Структура(
		"ВидБлока, Инициализация, КонецСчётчика, Тело",
		"Для", Инициализация, КонецСчётчика, Тело
	);
КонецФункции
...
Функция РазобратьЛогическуюСтроку()
	...
	Пока ТекущийИндекс < ВсегоТокенов Цикл
		...
		Если НЕ ЭтоИнструкцияПрепроцессора И ЗавершающиеТокены.Свойство(ТекущийТокен) Тогда
			// Если это точка с запятой, включаем её в логическую строку и увеличиваем индекс
			Если ТекущийТокен = "ТочкаСЗапятой" Тогда
				ТекущийИндекс = ТекущийИндекс + 1;
			КонецЕсли;
			Прервать;
		КонецЕсли;
...
Функция РазобратьДля()
	...
	КонецСчётчика = РазобратьЛогическуюСтроку();
	...
	Возврат СоздатьБлокДля(Инициализация, КонецСчётчика, Тело);
КонецФункции
